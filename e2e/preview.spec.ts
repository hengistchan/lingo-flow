...
test('options preview uses readable language selectors and functional navigation', async ({ page }) => {
  ...
  await expect(page.getByLabel('Interface theme')).toHaveValue('system')
  ...
  await page.getByLabel('Interface theme').selectOption('system')
  await expect(page.locator('html')).not.toHaveClass(/dark|light/)
  await page.getByRole('button', { name: 'Change in browser' }).click()
  await expect(page.getByText(/Could not open browser shortcut settings/)).toBeVisible()
  ...
  await expect(page.getByLabel('Fallback provider').locator('option[value="google-free-translate"]')).toHaveCount(0)
  ...
  await expect(page.getByLabel('Max cache items')).toHaveValue('50000')
  await page.getByLabel('Enable local translation cache').uncheck()
  await expect(page.getByLabel('Max cache items')).toBeDisabled()
